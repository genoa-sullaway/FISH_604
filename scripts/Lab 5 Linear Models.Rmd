...
******** HAVING TROUBLE WITH THIS ONE... CHECK SOLUTIONS. 
...
x_even<- log(c(seq(from = 100, to = 6000, by = 900)))

mammals.lm <- lm(log(brain)~log(body), data=mammals)

pred<- as.data.frame(predict(mammals.lm, newdata=data.frame(body= x_even)))
pred$log_x <- x_even

pred$log_y<- pred$`predict(mammals.lm, newdata = data.frame(body = x_even))`

#pred$log_y<- pred$`exp(predict(mammals.lm, newdata = data.frame(body = x_even)))` 

pred$x<-exp(pred$log_x)
pred$y<-exp(pred$log_y)


ggplot(mammals,aes(x=body, y = brain)) +
  geom_point( ) +
  geom_smooth(method = "lm") + 
  geom_point(data = pred,aes(x=x, y = y), color = "pink")
