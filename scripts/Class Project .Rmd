...
#filter out areas where it is clearly extrapolating. 
 cal<-cal_predict %>%
   filter(!se.fit>1.5) %>%
   rbind(cal_early) # DO A BETTER JOB AT FILTERING THESE FOR WHERE THERE IS LITTLE DATA, IE 2016 AND NORTHERN PORTION OF 2017
...
```{r PLOT JULIAN DAY COEFFICIENTS}
julian_predict <- expand_grid(YEAR = c("2016"),
                        julian = seq(min(df_model$julian), max(df_model$julian)), 
                        TEMPERATURE = 6,
                        #LAT = seq(min(df_model$LAT), max(df_temperature_model$LAT), length=20),
                        #LON = seq(min(df_model$LON), max(df_temperature_model$LON), length =20))
                         LAT = 61.2465,
                         LON = -173.7395)
 
output<- predict(best_calanus,julian_predict, se.fit = TRUE) 
 
output_df<-do.call(cbind.data.frame, output)
 
julian_predict_df <- cbind(julian_predict,output_df) %>%
  filter(julian < 170 |
         julian > 230 ) %>%
  mutate(group = case_when(julian <170 ~ "1",
                           julian > 230 ~ "2"))

#julian_calanus<-df_calanus %>% 
 # filter(YEAR == "2016")#,
         # LAT == 61.2465,
         # LON == -173.7395)
 
julian_calanus<-ggplot(julian_predict_df, aes(x=julian, y = fit)) +
  geom_line(aes(group = group), color = "blue", linetype = "dotted") +
  geom_ribbon(aes(ymin = fit-se.fit, ymax = fit + se.fit, group = group), alpha = 0.2) +
 # geom_point(data = julian_calanus, aes(x=julian, y = log(sum)), alpha = 0.5) +
  ylab("Log Estimated CPUE") + 
  xlab("Julian Day") + 
  ggtitle("Calanus")

########################################################
#######################################################
#Do it for Pseudocalanus

output<- predict(best_ps ,julian_predict, se.fit = TRUE) 
 
output_df<-do.call(cbind.data.frame, output)
 
julian_predict_df_ps <- cbind(julian_predict,output_df) %>%
  filter(julian < 165 |
         julian > 230) %>%
  filter(!julian < 125) %>%
  mutate(group = case_when(julian <170 ~ "1",
...
julian_ps<-df_psuedocalanus %>% 
  filter(YEAR == "2016")  
 
julian_psuedocalanus<-ggplot(julian_predict_df_ps, aes(x=julian, y = fit)) +
   # geom_point(data = julian_ps, aes(x=julian, y = log(sum)), alpha = 0.8,shape = 1, color = "darkblue" ) +
  ...
  xlab("Julian Day") + 
  ggtitle("Pseudocalanus")
julian_psuedocalanus

# julian_day_zoop<-ggarrange(julian_calanus, julian_psuedocalanus, labels = c("A", "B"))
# 
# pdf("final_project_figures/julian.pdf")
# julian_day_zoop
# dev.off()


#save temp plots
pdf("final_project_figures/calanus_temp.pdf")
calanus_temp
dev.off()

pdf("final_project_figures/pseudocalanus_temp.pdf")
pseudo_temp
dev.off()

pdf("final_project_figures/calanus_julian.pdf")
julian_calanus 
dev.off()

pdf("final_project_figures/pseudocalanus_julian.pdf")
julian_psuedocalanus 
dev.off()
...
```{r save pretty maps}
pdf("final_project_figures/model_pred_map.pdf")
ggarrange(calanus_pred_late,ps_map_late, labels = c("A", "B"), nrow =2)
dev.off()
