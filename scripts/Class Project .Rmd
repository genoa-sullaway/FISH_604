...
df_summary<- df %>%
  group_by(YEAR, LAT, LON, TEMPERATURE, SALINITY, DOMAIN, MAX_GEAR_DEPTH, BOTTOM_DEPTH, SE.wind.May.Sep, NW.wind.May.Sep, summer.cold.pool.extent) %>%
...
ggplot(ak) +
  geom_sf() +
  geom_point(data = df_summary, aes(x=LON, y=LAT)) + #, size = EST_NUM_PERM3))+
  theme_classic() +
  # scale_color_viridis() +
  # labs(color= "Log(CPUE)")  +
  #scale_color_gradient(low = "#de9b71", high = "#5d74a5", name = "Log C. marshallae CPUE") +
...
```{r Model: m1 year + temp}
m1 <- gam(sum ~ YEAR + s(TEMPERATURE, k=4), #s(YEAR, bs = 're', k=1),
         data = df_model, method = "ML", family = Tweedie(p=1.84)) #estimated this p from a simple model using tw
 
summary(m1)
plot(m1)
gam.check(m1)
plot(df_model$sum,fitted(m1))
...
new.df$pred <- predict(m1, newdata=new.df)
...
#plot(m1, scheme = 2, n = 200, tran = family(m)$linkinv)
#plot(m1, shade=T, resid=T, pch=16)
#family(m1) #check family and link function, this is log link. 
...
```{r Model: m2 year+temp+gaussian spatial}
m2 <- gam(sum ~ YEAR + s(TEMPERATURE, k=4) + s(x, y, bs='gp',m=2), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))

summary(m2)
AIC(m1, m2) #including spatial component improves fit by a lot
plot(m2)

plot(df_model$sum,fitted(m2))
...
- adding wind vs adding year = exact same AIC.
- pretty significant aic decrease with spatial gaussian

```{r Model: m3 wind+temp+gaussian spatial}
m3 <- gam(sum ~ SE.wind.May.Sep + NW.wind.May.Sep+ s(TEMPERATURE, k=4) + s(x, y, bs='gp',m=2), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))

summary(m3)
AIC(m1, m2,m3) #including spatial component improves fit by a lot
plot(m3)
...
# salinity and depth improve fit but are a minor addition
# weirdness for the salinity and depth plots ... not sure why

```{r Model: m4 year+temp+salinity+gaussian spatial}
m4 <- gam(sum ~ YEAR + s(TEMPERATURE, k=4) +   s(SALINITY, k =4 )  +
            s(x, y, bs='gp',m=2), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))
...
# m5 is the winner so far
# need to predict values and check more goodness of fit? -- weirdness in depth predictor
# compare the GP power to exponential and Matern covariance structures, see how they differ, also see if I can plot variogram for these other covariance structures.
# in the r smoother for cov structure: order of m:between 1 and 5 selects the correlation function from respectively, spherical, power exponential, and Matern 
# next need to look at plots for inspo, map a prediction/year, draw conclusions

```{r function to plot predicted values}
# To facilitate plotting the output for different models, it pays to
# to write a quick function to do so:
plot.calanus <- function(Model, Data=df_model) {
  Model <- m5
  # 'Model' should be a model with an appropriate 'predict' method
  dat <- data.frame(expand_grid(YEAR = factor(c("2014", "2015", "2016")),
                                SALINITY = seq(29, 32.8, by=0.5),
                                MAX_GEAR_DEPTH = seq(12, 250, by=10),
                                TEMPERATURE = seq(3,12, by=0.5),
                                x = seq(min(df_model$x), max(df_model$x), by = 0.5),
                                y = seq(min(df_model$x), max(df_model$x), by = 0.5)))

  Fitted <- predict(Model, newdata=dat, se=T)
  dat$mu <- exp(Fitted$fit)  # predicted values on response scale
  dat$lower <-  exp(Fitted$fit - 1.96 * Fitted$se.fit)
  dat$upper <-  exp(Fitted$fit + 1.96 * Fitted$se.fit)
 
  
  ggplot( ) +
    geom_point(data = df_model, aes(x=sum, y = TEMPERATURE) ) +
    facet_wrap(~YEAR)+
    #geom_ribbon(data = dat, aes(x= mu, ymin = lower, ymax= upper)) +
    geom_line(data = dat, aes(x=mu, y = TEMPERATURE))
...
```{r Model: m5 year+temp+Gear_depth+gaussian spatial}
 
m5 <- gam(sum ~ YEAR + s(TEMPERATURE, k=7 ) +  
            #s(SALINITY, k =4 ) +# taking salinity out for now, it does slightly improve AIC, but I dont have a hypothesis around why salinity is important 
            s(log(MAX_GEAR_DEPTH), k =5) +
            s(x, y, bs='gp', m=2), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))

summary(m5)
# 
AIC(m1, m2,m3,m4,m5) #including spatial component improves fit by a lot
b<-getViz(m5)
...
concurvity(m5) # to me this looks like salinity and max depth have high concurvity? 
concurvity(m5, full = FALSE) #but it seems kind of unclear when I look at the pairwise

#plot(df_model$sum,fitted(m5))
gam.check(m5)
...
plot(b, select = 3) + l_dens(type = "cond") + l_fitLine() + l_ciLine()
...
gridPrint(check1D(b, "x") + l_gridCheck1D(gridFun = sd, showReps = TRUE), 
          check1D(b, "y") + l_gridCheck1D(gridFun = sd, showReps = TRUE), ncol = 2)

######resdiuals from franz scripts
plot(df_model$YEAR, resid(m5), type="b"); abline(h=0, col=4)
plot(df_model$TEMPERATURE, resid(m5), type="b"); abline(h=0, col=4)

plot(m5, residuals = TRUE, pch = 1, cex = 1)

plot(m5, pages = 1, all.terms = TRUE)

plot(m5, select = 1, shade = TRUE, shade.col = "hotpink", 
     shift = coef(m5)[1], seWithMean = TRUE)

plot(m5, select = 2, shade = TRUE, shade.col = "hotpink", 
     shift = coef(m5)[2], seWithMean = TRUE)

plot(m5, scheme = 1, pages = 1)
 
#visualize spatial uncertainty
vis.gam(m5, view = c("x", "y"), 
...
vis.gam(m5, view = c("x", "y"), 
        plot.type = "contour", too.far = 0.15)
...
df<-data.frame(x = seq(from=min(df_2014$x), 
              to=max(df_2014$x),
              length.out = 424),
  y = seq(from=min(df_2014$y),
              to=max(df_2014$y),
              length.out = 424))
...
pred_2014<- df_model %>% 
  filter(YEAR == 2014) %>%
  ungroup() %>%
  dplyr::select(YEAR,TEMPERATURE,MAX_GEAR_DEPTH, LAT, LON, x,y) #%>%
  #cbind(df)

# pred_2014 <- expand_grid(
#    YEAR = rep("2014"),
#    TEMPERATURE = seq(from=min(df_2014$TEMPERATURE), 
#               to=max(df_2014$TEMPERATURE), 
#               length.out = 20),
#     MAX_GEAR_DEPTH = seq(from=min(df_2014$MAX_GEAR_DEPTH), 
#               to=max(df_2014$MAX_GEAR_DEPTH), 
#               length.out = 20),
#   x = seq(from=min(df_2014$x), 
#               to=max(df_2014$x), 
#               length.out = 50),
#   y = seq(from=min(df_2014$y), 
#               to=max(df_2014$y), 
#               length.out = 50))
 ...
 pred2014_df <- predict(m5, newdata =  pred_2014, 
                      se.fit = TRUE) %>%  
  as_tibble() %>% 
  cbind(pred_2014)
...
#   df_model$y <- x$y
 ...
 ggplot(pred2014_df, aes(x=x, y=y, fill =  fill)) + 
  #geom_hex(aes(fill = fit, color = fit)) 
  stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE)  

  ggplot(pred2014_df) + 
    geom_point(aes(x=fit,  y = TEMPERATURE))

```

# Gear depth offers more than bottom depth...

```{r Model: m6 year+temp+salinity+bottom_depth+gaussian spatial power}
m6 <- gam(sum ~ YEAR + s(TEMPERATURE, k=4) +  
            s(SALINITY, k=4 ) + 
            s(BOTTOM_DEPTH, k =4) +
            s(x, y, bs='gp',m=2), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))
...
summary(m6)
AIC(m1, m2,m3,m4,m5,m6) #including spatial component improves fit by a lot
...
```{r Model: m7 year+temp+Gear_depth+gaussian spatial exponential}
m7 <- gam(sum ~ YEAR + s(TEMPERATURE, k=7 ) +  
            #s(SALINITY, k =4 ) +# taking salinity out for now, it does slightly improve AIC, but I dont have a hypothesis around why salinity is important 
            s(log(MAX_GEAR_DEPTH), k =5) +
            s(x, y, bs='gp', m=3), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))
...
AIC(m1, m2,m3,m4,m5,m7) #including spatial component improves fit by a lot
b<-getViz(m7)
...
```{r Model: m8 year+year_random_effect+temp+Gear_depth+gaussian spatial}
m8 <- gam(sum ~ YEAR + s(YEAR, bs="re") + s(TEMPERATURE, k=7 ) +  
            #s(SALINITY, k =4 ) +# taking salinity out for now, it does slightly improve AIC, but I dont have a hypothesis around why salinity is important 
            s(log(MAX_GEAR_DEPTH), k =5) +
            s(x, y, bs='gp', m=2), 
         data = df_model, method = "ML", family = Tweedie(p=1.84))
