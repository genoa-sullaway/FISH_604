...
#    variogram. Don't pay too much attention to the observations at the largest 
...
# Our goal is to estimate a smoothed surface that best represents
# temperatures throughout the region

#### 1. Generalized Additive Model to fit a smooth surface
# We can easily fit a simple GAM smoother. Here I use a tensor 
# product interaction with somewhat more flexibility in the 
# longitudinal direction (stronger onshore-offshore gradients
# than north-south gradients?):
temp.gam <- gam(Temp.surf ~ te(longitude,latitude, k=c(8,5)), data=env10)
summary(temp.gam)
# The smooth uses about 26 equivalent parameters
# For a quick visualization that is a bit better than 'vis.gam', 
# we can use the 'gratia' or 'mgcViz' package:
library(gratia)
draw(temp.gam) + 
  geom_point(aes(longitude,latitude), data=env10, color="grey60")
# I added the actual data locations so you can assess the extent of
# extrapolation

library(mgcViz)
pl <- getViz(temp.gam)
plot(sm(pl, 1)) + l_fitRaster() + l_fitContour() + l_points()

# Examine residuals from the gam model:
env10$r <- resid(temp.gam)  # Extract residuals
ggplot(env10) +
  geom_point(aes(longitude,latitude, color=r, size=r)) + 
  scale_color_gradient2()
# The residual patterns looks fairly good and the GAM surface
# appears to provide a very reasonable approximation to the 
# spatial temperature pattern in the norther Bering Sea


### 2. Geostatistical approach to estimating the temperature surface:

# First, we need to find any duplicates in the location. If there are any, 
# we need to 'jitter' those locations slightly or use the mean of observations
# at the same location because the variogram functions cannot have zero 
# distances between any two samples:
...
# The 'get.borders' function finds the convex hull (function 'chull') and
# extends it slightly beyond the data based on the 'frac' value. Setting
# 'frac=0' is identical to using chull:
chull.sub <-  chull(dat10[,1:2])
dat10[chull.sub,1:2]  
# This is identical to results from:
get.borders(dat10[,1:2], frac=0)
...
# Estimate variogram using a simple weighted least squares approach with 
...
# of pairs in each bin as weights. As such, this is an ad hoc approach to 
# finding reasonable starting values for a more suitable maximum likelihood
# approach as implemented below.

# Output:
dat10.var.fit
# In the output, 'tausq' is the nugget effect ('nugget variance'), 'sigmasq'
# is the sill and 'phi' is the range parameter
...
# This approach assumes stationarity, but there is a strong trend in
# surface temperatures across the NBS
...
#  mu(x)= a + b1*x + b2*y + b3*x^2 + b4*y^2 + b5*x*y
...
(cp <- dat10.reml2$cov.pars)
(nug <- dat10.reml2$nugget)
kr.obj <- krige.control(type.krige="OK", trend.d = "2nd", trend.l = "2nd",
                           cov.model="exponential",
                           cov.pars=cp, nugget=nug)  
...
# Make spatial predictions over the grid:
