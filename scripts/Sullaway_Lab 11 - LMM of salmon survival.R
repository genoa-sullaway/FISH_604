...
spawn <- 299 #long term mean spawners
...
# SOlVE: log(R/S)  = log(predicted_above)
r_below = exp(predicted_below) * spawn
r_above = exp(predicted_above) * spawn

r_above -r_below 

# Average recruitment increases by about 377,000 recruits with a 2 degree temperature anomoly change 
...
#The average productivity across stocks, a, is 1.15  with a standard error of +/- .105.  
 
# 2. What is the variability of the productivity parameter across stocks?
#Stock productivity varies 0.22 across stocks. 

#note: got this from VarCorr(sm), Intercept value, representing a in the model. 

# 3. How does SST affect the response {log(R/S)} and how much does the 
#    estimated effect vary across stocks? 

# Positive relationship between SST and log(R/S), (shown by SST estimate in summary(sm)), 
# variance across stocks is 0.004, so overall this seems like low variance 
# (this is based on the SST variance in the Varcorr matrix).  

#Ask follow up Q is about the correlation column in the VarCorr matrix**
#---------------------------------------------------------------------------
...
# Fit 2 is the best model here, though fit 1 and 2 are pretty similar. 
# This indicates that SST is important in predicting West Coast pink salmon 
# survival. Fit 2 does not include a random effect of SST, 
# indicating that salmon survival has a positive relationship with SST, but the variation 
#across stocks is not super important to include. 
...
#logrs <- a +a.nome - beta*S + gamma*SST + g.nome*SST.nome
logrs_below <- cf.fix[1] + cf.rand[1] + cf.fix[3]*S + cf.fix[2]*-1 + cf.rand[2]*-1
logrs_below

logrs_above <- cf.fix[1] + cf.rand[1] + cf.fix[3]*S + cf.fix[2]*1 + cf.rand[2]*1
logrs_above

r_above = exp(logrs_above) * S
r_below = exp(logrs_below) * S
r_above - r_below

#Here we predict a difference in salmon productivity of 322,000 with a 2 degree c increase. 
...
# The REML model has a lower predicted difference than the Exercise 1 model. 
...
library(lme4)
fit1.REML
(reml <- summary(fit1.REML)) 

#    Fit via 'ML' to allow for model comparisons
fit1.lmer <- lmer(SR ~ SST + stock:S + (SST | stock), data = pinks,
             REML = TRUE)
(lmer <- summary(fit1.lmer))  # Somewhat verbose (as it includes var-cov matrix)
...
#LMER and NLME produce very similar if not identical outputs. 
