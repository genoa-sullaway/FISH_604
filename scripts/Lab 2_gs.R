...
log_norm_mean=exp((mu+VAR/2))

log_norm_var=(exp(VAR)-1)*exp(2*mu+VAR)
##################################################
...
M2 <- read.csv("data/M2winds.csv", as.is=T)  # 'as.is' suppresses importing of Date column as a factor 
...
library(tidyverse)
M2.tbl <- read.csv("data/M2winds.csv")
...
# Interpret and discuss the output with your group!
    ...
    # More variable wind speeds in Winter, less variability in summer generally lower wind speeds. 
...
# ------ Solution-----------------------------------------------------
quantile(W, probs = 0.90, names=TRUE)
# --------------------------------------------------------------------
...
for(i in 1:999) {
  ...
  out[i] <- quantile(W.boot, probs = 0.90, names=TRUE)
}

mean(out)  #Bootstrap estimate of median
sqrt(sd(out)) # Bootstrap estimate of standard error of median
...
#by-hand:
n=8
(factorial(n)/(factorial(1)*factorial(4)*factorial(3)))*(p[1]^1)*(p[2]^4)*(p[3]^3)
...
# ------ Solution-----------------------------------------------------
mat<-rmultinom(100,size=50,prob=p) 
rowMeans(mat)

apply(mat,MARGIN =1, mean)
apply(mat,MARGIN =1, sd)
