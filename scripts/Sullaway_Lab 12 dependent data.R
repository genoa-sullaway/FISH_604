...
fit.gls4 <- gls(Y ~ X, correl = corARMA(p=4), method="ML")
summary(fit.gls4)
#---------
...
#---------
lrtest(fit.gls, fit.gls4)

#the model with AR-4 is preferred in the likelihood ratio rest. 
#---------
...
fits<-list(fit.lm, #0.009469 0.0003679
        fit.gls,   #0.009535 0.0007494 
        fit.gls4)  #0.009618 0.0012079  

names<-c("lm", "ar1", "ar4")

names(fits) <- names

output<-purrr::map(fits, summary)
#purrr::map(fits,c(2, 2))

# What can you conclude? 
#---------
# When we account for AR(4) the slope is barely steeper and the SE is wider. 
#---------
...
#----------------
# Your answer:
# The result suggests that global temperature change is non-linear (edf of 6.3).
# The advantage of GAM is that it captures the non-linear changes through time, ie the 
# more recent increase in CO2 that is correlated with global temperature. 
#----------------
...
plot(Global.temp$Year, resid(fit.gam), type = "b"); abline(h=0, lty=2)
plot(Global.temp$Year, resid(fit.gls), type = "b"); abline(h=0, lty=2)
plot(Global.temp$Year, resid(fit.lm), type = "b"); abline(h=0, lty=2)

# Extract and save residuals:
r <- resid(fit.gam)
n <- length(r)   # number of years

lag.plot(r, set.lags=4, do.lines=F, col=4, pch=16, diag.col=2)
pacf(r, lag.max=25)
...
#---------------
#the GAM did a better job at accounting for the mean temporal trend compared to
#other models we have used here. The autocorrelation is down to ~.20 based on the pacf which
# is an improvement form other models, however it is still outside the confidence band for lag-1
#But based on ther durbin watson test there is still eveidence of significant autocorrelation. 
#---------------
...
fit.gamm4 <- gamm(Y ~ s(X), correlation = corARMA(p=4))
...
AIC(fit.gamm0$lme, fit.gamm$lme, fit.gamm4$lme)
...
### ANSWER
# Because the CI does not cross zero, this means there is more AR we should account for
